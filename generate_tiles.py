...
def save_tiles(tiles, chessboard_img_path):
    ...
    for i in range(64):
        ...
        save_path = 'sub_dir/{}'.format(piece)

        if piece == blackrook:
            save_path = "sub_dir/rr"
        if piece.__eq__(blackknight):
            save_path = 'sub_dir/nn'
        if piece == blackbishop:
            save_path = 'sub_dir/bb'
        if piece == blackqueen:
            save_path = 'sub_dir/qq'
        if piece == blackking:
            save_path = 'sub_dir/kk'
        if piece == blackpawn:
            save_path = 'sub_dir/pp'
        ...
        # tile_img_filename = '{}/{}/{}_{}.png'.format(sub_dir,piece, piece, y)
        tile_img_filename = '{}/{}_{}.png'.format(save_path, piece, y)
...
def generate_tiles_from_all_chessboards():
    ...
    for i, chessboard_img_path in enumerate(chessboard_img_filenames):
        ...
        if os.path.exists(img_save_dir) and not OVERWRITE:
            ...
            num_skipped += 1
            continue
        tiles = get_chessboard_tiles(chessboard_img_path, use_grayscale=False)
